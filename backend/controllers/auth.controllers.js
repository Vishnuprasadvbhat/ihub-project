...
export const IsAuthenticated = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.json({ success: false, isVerified: false, message: 'User not found' });
    }
    
    if (!user.isVerified) {
      return res.json({ success: false, isVerified: false, message: 'Email not verified' });
    }
    
    return res.json({ success: true, isVerified: true, message: 'User authenticated' });
  } catch (error) {
    return res.status(500).json({ success: false, isVerified: false, message: error.message });
  }
};
